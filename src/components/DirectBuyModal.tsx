...
export const DirectBuyModal: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Step 2: Customer Account Deduction
    setTimeout(() => {
      setProcessingStep(3);
    }, 1800);

    // Step 3: Admin Settlement
    setTimeout(() => {
      setProcessingStep(4);
    }, 2800);
  ...
  return (
    <div className="modal-backdrop" onClick={() => !isSubmitting && setDirectBuyItem(null)}>
      <div 
        ...
        {transactionFailed ? (
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>
              {processingStep === 1 && `Connecting to Gateway...`}
              {processingStep === 2 && `Deducting ${formatPrice(unitPrice)} from Account...`}
              {processingStep === 3 && `Transferring Funds to Admin Account...`}
              {processingStep === 4 && `Payment Deducted & Order Confirmed!`}
            </h3>

            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: '1.5rem', maxWidth: '420px' }}>
              256-Bit Encrypted Transfer via <strong style={{ color: 'var(--accent-cyan)' }}>{paymentType === 'paypal' ? 'PayPal Direct' : selectedNetwork}</strong>. Please hold on.
            </p>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.84rem' }}>
                <span style={{ color: processingStep >= 1 ? 'var(--text-primary)' : 'var(--text-muted)', fontWeight: processingStep >= 1 ? 700 : 400 }}>
                  1. Payment Gateway Handshake
                </span>
                {processingStep > 1 ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.84rem' }}>
                <span style={{ color: processingStep >= 2 ? 'var(--text-primary)' : 'var(--text-muted)', fontWeight: processingStep >= 2 ? 700 : 400 }}>
                  2. Deducting {formatPrice(unitPrice)} from Customer Account
                </span>
                {processingStep > 2 ? (
                  <CheckCircle2 size={16} color="var(--accent-emerald)" />
                ) : processingStep === 2 ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.84rem' }}>
                <span style={{ color: processingStep >= 3 ? 'var(--text-primary)' : 'var(--text-muted)', fontWeight: processingStep >= 3 ? 700 : 400 }}>
                  3. Settlement to Admin Merchant Account
                </span>
                {processingStep > 3 ? (
                  <CheckCircle2 size={16} color="var(--accent-emerald)" />
                ) : processingStep === 3 ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.84rem' }}>
                <span style={{ color: processingStep >= 4 ? 'var(--accent-emerald)' : 'var(--text-muted)', fontWeight: processingStep >= 4 ? 800 : 400 }}>
                  4. Hardware Order Confirmation Receipt
                </span>
                {processingStep >= 4 ? (
                  <CheckCircle2 size={16} color="var(--accent-emerald)" />
                ) : (
